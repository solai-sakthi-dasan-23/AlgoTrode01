                        ...
                        const MqlTradeResult &result)
{
   ...
   ulong deal_magic = (ulong)HistoryDealGetInteger(deal, DEAL_MAGIC);
   string deal_comment = HistoryDealGetString(deal, DEAL_COMMENT);
   // Some brokers report zero magic on an exit deal, so accept our order
   // comment as a fallback. This also covers every position in the stack.
   if(deal_magic != InpMagic && StringFind(deal_comment, "AT01|STACK") < 0) return;
   ENUM_DEAL_REASON reason = (ENUM_DEAL_REASON)HistoryDealGetInteger(deal, DEAL_REASON);
   Print("AlgoTrade01 exit: ", EnumToString(reason), " volume=",
