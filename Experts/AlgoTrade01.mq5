...
input int    InpStopLossPips         = 20;         // Initial order stop loss
...
double PriceForSL(ENUM_POSITION_TYPE type, double entry)
{
   double distance = InpStopLossPips * Pip();
...
bool OpenInitial(ENUM_ORDER_TYPE order_type)
{
   ...
   double sl = PriceForSL(order_type == ORDER_TYPE_BUY ? POSITION_TYPE_BUY : POSITION_TYPE_SELL, price);
   string comment = "AT01|INIT";

   trade.SetExpertMagicNumber(InpMagic);
   trade.SetDeviationInPoints(InpDeviationPoints);
   bool sent = order_type == ORDER_TYPE_BUY
               ? trade.Buy(InpLots, _Symbol, 0.0, sl, 0.0, comment)
               : trade.Sell(InpLots, _Symbol, 0.0, sl, 0.0, comment);
   if(!sent)
   {
...
void ManageStrategy()
{
   ...
   if(!IsOurPosition(initial_ticket))
   {
      if(!intentional_stop)
      {
         ...
         Print("Initial position closed (normally the stop loss); closing basket and reversing.");
         CloseOurPositions();
         ResetState();
